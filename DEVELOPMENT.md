...
修改降智判据、资源风控或复核行为前，先读[质量模块中的 A/B 静默路由定义、四种资源组合与脱敏对照](backend/internal/quality/README.md#降智的含义与四种资源组合)。实时守卫与主动检测共享协议事实，不共享案件判决；超时、可用性和缺少可见思考必须分别解释。
...
Build 的 `providerBuild.sessionIdleConnTimeout` 是管理面持久化热设置，不从 YAML 或环境变量读取；默认 `5m`，范围 `30s`–`30m`。旧持久载荷缺少字段时补默认，旧 HTTP 客户端缺省或空字符串提交保持现值，显式零和越界值拒绝。SQLite/PostgreSQL 沿用运行设置 JSON 与 revision/CAS，不新增 schema、不清理业务数据。新请求使用新连接配置，旧活跃请求保留原资源直至完成；改变此值可能让下一次请求重新建连。不同实例沿用现有 apply/通知收敛机制，不能承诺跨实例原子切换；旧二进制忽略该字段并使用原固定值，回退或混合版本不保证相同保留时间。
...
模型与客户端 Key 编辑器采用相同合同：保存基线和 dirty 字段属于当前对象，关闭再打开同一 ID 也不能复用旧请求。批量命令在提交时携带确定的 ID 集合，结果只移除本次处理的选择；对象操作的迟到结果不能关闭另一个对象的确认框。列表返回时强制读取权威状态。模型同步的进度订阅与服务端已接受的任务分开，离页只终止订阅；再次进入通过同步状态查询恢复显示。
...
PostgreSQL 主库初始化按迁移协调锁、固定顺序的 Provider 模型命名空间锁、表/行锁的顺序执行。命名空间锁必须在 DDL 前取得，与模型目录写入顺序一致，防止一个实例初始化目录时与另一个实例的启动迁移互相等待。
...
历史准备借用已验证 JSON 的根字段和输入项，逐项计算原有可见历史哈希；规范化工作区在数据库历史加载前释放。恢复只读取项类型和需要补齐的推理内容，不为类型判断重复编码媒体；无需补齐时保留输入请求字节。Build 的 `reasoning_text` 兼容修补只改推理项，其他输入项和大整数保持原始 JSON 数值。这些优化不改变历史哈希、scope、加密格式、schema 或 normalizer 版本，无需迁移或回填。大输入预算与失败分类见[守卫资源合同](backend/internal/quality/guard/README.md#资源与验证边界)。
...
代理池 `strategy` 新增 `session-reuse`（管理端“会话复用”），沿用现有字符串列。SQLite/PostgreSQL 在现有启动迁移事务内扩展 `chk_egress_pools_strategy`，同时补齐已支持的 `least-used` 值；无新增表、列或业务回填。SQLite 重建约束使用既有外键暂停与完整性检查，PostgreSQL 使用既有迁移锁；重复启动不再重建已升级约束。现有策略和空值默认保持原行为。新策略按 Build 会话在单实例内分配出口，优先当前在途请求较少、保留会话绑定较少的合格节点；后续请求保持绑定，账号切换不主动重绑，质量羁押、排除和成员变化仍优先。详见 [网络模块说明](backend/internal/infra/egress/README.md#session-reuse-pool-strategy)。

管理前后端应一起升级后再保存新策略；旧管理端解码器不认识该枚举，旧后端会将未知策略归一为历史 affinity 行为。回退前将使用 `session-reuse` 的池显式改为旧版本支持的策略；不要依赖混合版本保持会话分配一致。会话映射仅是有界内存提示，重启清空、空闲两小时过期，不修改持久历史或计费。多实例共享绑定和负载协调尚未实现，不应把单实例结果外推为跨实例保证。

### 审计缓存用量的兼容要求

审计详情的可选 `diagnostics` 保留有限阶段累计时长、每次实际 HTTP 调用的网络事件及 Build 最终输入组件摘要。`request_audits.diagnostics_json` 是 SQLite/PostgreSQL 可空文本列，由现有启动迁移增加；旧记录保持 NULL，不回填。列表查询不读取该列，详情在原有授权和一致性快照中读取。待写日志随审计保留这个可选值，旧程序可忽略新列，回退不删除诊断或业务数据。单条诊断最多 32 个阶段、32 次物理调用、每次 32 个网络事件和 8 个失败分类，序列化上限 512 KiB；达到采集上限明确标记截断。

缓存组件使用进程随机密钥的 HMAC，仅保存摘要与密钥代际，不保存正文、密文、地址、认证头或密钥。只有同一代际可比较；重启和不同实例之间不可比较。输入链摘要覆盖前 32 项与末 32 项；缺少的边界保持未知，不能推断前缀相同。摘要对应最终发出的字节，包含推理项，字节变化不必然意味着语义变化，相同摘要也不保证上游命中。网络事件相对每次物理提交计时，包含内部连接重试；阶段存在包含关系，不应直接相加。响应头等待无法区分上游排队、预填充与模型计算。入口至审计前的时长不包含审计自身写入，也不能证明客户端已接收全部字节。

必要历史提交仅对存储边界明确分类的连接、锁、序列化及存储超时故障重试，最多三次、退避 50/150 ms，共享原有五秒期限。每次使用相同 ticket、response ID 和输出，重新检查当前代际、版本及租约到期；已提交的相同结果幂等成功。冲突、容量、历史失效、取消与未知错误不自动重试，不重发上游生成。质量事件回执在原有调用期限内对同类瞬态错误最多补试一次，保留事件时间、持有者和保护期限；现有完成义务恢复继续处理长期失败。
...
Build 网络阶段采样可单独设置 `GROK2API_UPSTREAM_NETWORK_TRACE_DIR`，记录连接申请、DNS/TCP/TLS、连接复用、TLS 会话恢复、请求写完和首个响应字节的时间，不保存地址、凭据或正文。`GROK2API_UPSTREAM_TRACE_DIR` 保留原有完整追踪行为，同时保存请求/响应内容；需要仅采集网络阶段时必须取消该完整追踪变量。两者均在进程首次使用时读取，修改需重启。目录创建失败时禁用相应采样，诊断文件没有自动保留期，使用私有目录并限制采样时段。该采样不能单独区分上游排队与模型计算时间。
