...
Then set the Grok Web Clearance mode to `FlareSolverr` in the admin UI and use `http://flaresolverr:8191` as the solver URL. FlareSolverr is not published on the host; each Web egress node uses its own proxy to obtain cookies and User-Agent.
