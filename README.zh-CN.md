...
- **可选账号自动清理**（默认关闭）：运行设置可按间隔硬删除已标记 `reauthRequired` 且 `reauth_marked_at` 超过最短保留时长的账号。不会选中纯冷却账号，也不会打断仍可 drain 的永久 refresh 账号；有进行中视频任务的账号会被跳过（不阻塞其余清理）。启用后与进程启动后的首次扫描均等待一个间隔；热更新只重排下一次扫描，不会立即删除。
