...
- **可选账号自动清理**（默认关闭）：运行设置可按间隔硬删除已标记 `reauthRequired` 且 `reauth_marked_at` 超过最短保留时长的账号。不会选中纯冷却账号，也不会打断仍可 drain 的永久 refresh 账号；有进行中视频任务的账号会被跳过（不阻塞其余清理）。进程启动后首次扫描等待一个间隔。
