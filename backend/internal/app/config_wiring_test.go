...
// requestRetry 段经 config.Load 与 qualityGuardConfig 映射后，运行时结构与
// 文档值逐字段一致。此前只有子结构解码测试——字段改名/映射漂移不会
// 被发现（测试质量审计 P2#7）。
func TestExampleConfigQualityRetryWiring(t *testing.T) {
	...
	runtime := qualityGuardConfig(cfg.RequestRetry)
	...
	if runtime.ExhaustionPolicy() != wants.onExhausted {
		t.Errorf("OnExhausted = %q", runtime.ExhaustionPolicy())
	}
	...
	// Empty file selection uses the domain bootstrap defaults.
	if len(runtime.GuardedModels) != 2 || !runtime.Jurisdiction("grok_build", "grok-4.6") {
		t.Errorf("example jurisdiction = %#v", runtime.GuardedModels)
	}
	if runtime.AdmissionTimeout != 30*time.Second || runtime.ToolAdmissionTimeout != 3*time.Minute {
		t.Errorf("example admission budgets = %s/%s", runtime.AdmissionTimeout, runtime.ToolAdmissionTimeout)
	}
	...
	scopedRuntime := qualityGuardConfig(scopedCfg.RequestRetry)
