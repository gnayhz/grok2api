...
func replaceQuotaWindowsFixture(repo repository.AccountRepository, ctx context.Context, id uint64, tier account.WebTier, now time.Time, windows []account.QuotaWindow) error {
	revision, err := repo.GetQuotaRevision(ctx, id)
	if err != nil {
		return err
	}
	return repo.SaveQuotaSnapshot(ctx, repository.QuotaSnapshotWrite{AccountID: id, Revision: revision, Tier: tier, SyncedAt: now, Windows: windows, ReplaceAll: true})
}
