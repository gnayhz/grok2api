...
func BenchmarkSyncHygienePreservesCurrentTargets(b *testing.B) {
	for _, dialect := range []string{"sqlite", "postgres"} {
		b.Run(dialect, func(b *testing.B) {
			...
			b.ResetTimer()
			for range b.N {
				if err := service.enforceRoutingHygieneAfterSync(ctx, repo); err != nil {
					b.Fatal(err)
				}
			}
			b.StopTimer()
			...
			if after.DefaultTarget != target || after.ScopeTargets[domain.ScopeBuild] != target || after.ClassTargets[domain.TrafficClassInference] != target || !after.UpdatedAt.Equal(before.UpdatedAt) {
