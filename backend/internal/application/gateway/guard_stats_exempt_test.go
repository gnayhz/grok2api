...
func TestQualityHoldExemptReasonPaths(t *testing.T) {
	...
	if reason := qualityHoldExemptReason(input, nil, route, audit.OperationChat, cfg, nil); reason != "" {
	...
	if reason := qualityHoldExemptReason(injected, nil, route, audit.OperationChat, cfg, nil); reason != "" {
		t.Fatalf("injected search tools must still hold, got %q", reason)
	}
	off := cfg
	off.Enabled = false
	if reason := qualityHoldExemptReason(input, nil, route, audit.OperationChat, off, nil); reason != QualityExemptDisabled {
		t.Fatalf("disabled = %q", reason)
	}
	skip := input
	skip.skipQualityHold = true
	if reason := qualityHoldExemptReason(skip, nil, route, audit.OperationChat, cfg, nil); reason != QualityExemptSkipInput {
	...
	for _, op := range []audit.Operation{
		...
		if reason := qualityHoldExemptReason(input, nil, route, op, cfg, nil); reason != QualityExemptOperation {
			t.Fatalf("non-reasoning op %q = %q, want operation", op, reason)
		}
	}
	tui := input
	tui.Body = []byte(`{"input":[{"role":"user","content":"` + tuiCompactionPrompt + `"}]}`)
	if reason := qualityHoldExemptReason(tui, nil, route, audit.OperationResponses, cfg, nil); reason != QualityExemptCompaction {
		t.Fatalf("tui compaction body = %q", reason)
	}
	tuiSkip := tui
	tuiSkip.skipQualityHold = true
	if reason := qualityHoldExemptReason(tuiSkip, nil, route, audit.OperationResponses, cfg, nil); reason != QualityExemptSkipInput {
	...
	if reason := qualityHoldExemptReason(input, nil, webRoute, audit.OperationChat, cfg, nil); reason != QualityExemptProvider {
	...
	if reason := qualityHoldExemptReason(none, nil, route, audit.OperationChat, cfg, nil); reason != "" {
	...
	if reason := qualityHoldExemptReason(none, nil, route, audit.OperationChat, scoped, nil); reason != "" {
	...
	if reason := qualityHoldExemptReason(outOfScope, nil, route43, audit.OperationChat, scoped, nil); reason != QualityExemptModelScope {
	...
	if reason := qualityHoldExemptReason(alias, nil, route, audit.OperationChat, scoped, nil); reason != "" {
		t.Fatalf("effort-suffixed alias of in-scope model = %q, want gated", reason)
	}
	// 修正：流式 messages 未请求 thinking 照常 hold（转换器以
	// ThinkingEvidenceComment 保留证据）；非流式检查转换前的原始响应。
	if reason := qualityHoldExemptReason(input, nil, route, audit.OperationMessages, cfg, nil); reason != "" {
	...
	if reason := qualityHoldExemptReason(nonStream, nil, route, audit.OperationChat, cfg, nil); reason != "" {
		t.Fatalf("non-stream chat must hold, got %q", reason)
	}
	if reason := qualityHoldExemptReason(nonStream, nil, route, audit.OperationMessages, cfg, nil); reason != "" {
	...
	if reason := qualityHoldExemptReason(nonReasoningInput, nil, nonReasoning, audit.OperationChat, cfg, nil); reason != QualityExemptModelNoReasoning {
		t.Fatalf("non-reasoning model = %q", reason)
	}
	if shouldHoldQualityStream(input, nil, route, audit.OperationChat, cfg, nil) != (qualityHoldExemptReason(input, nil, route, audit.OperationChat, cfg, nil) == "") {
