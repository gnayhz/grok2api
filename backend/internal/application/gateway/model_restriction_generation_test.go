	...
	for _, scenario := range []string{"quota_reset_preserves_same_model_denial", "late_quota_after_reset", "old_denial_after_reimport"} {
		t.Run(scenario, func(t *testing.T) {
			...
			case "quota_reset_preserves_same_model_denial":
				if err := sel.MarkModelAccessDenied(ctx, v, "same-model", time.Hour); err != nil {
					t.Fatal(err)
				}
				sel.MarkModelQuotaExhausted(ctx, v, nil, "same-model", 24*time.Hour)
...
func TestModelRestrictionFactsSurviveClientCancellation(t *testing.T) {
	...
	cancel()
	if err := sel.MarkModelAccessDenied(canceled, v, "model", time.Hour); err != nil {
		t.Fatal(err)
	}
	sel.MarkModelQuotaExhausted(canceled, v, nil, "model", 24*time.Hour)
