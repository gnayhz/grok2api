				...
				for _, path := range []string{"ordinary", "session", "pinned", "sticky"} {
					t.Run(runtime+"/"+path, func(t *testing.T) {
						...
						if session != nil {
							...
							lease, err = selector.AcquirePinned(ctx, v.Provider, v.ID, 0, "grok-test", "", true)
						} else {
							lease, err = selector.Acquire(ctx, v.Provider, 0, "grok-test", "", affinity, excluded, false)
						}
						if lease != nil {
							lease.Release()
						...
						release()
						lease, err = selector.Acquire(ctx, v.Provider, 0, "grok-test", "", "", excluded, false)
