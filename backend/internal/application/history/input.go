...
//
// 调用方为每轮独立搜索,已占用的锚点从其下一位置继续搜索。这样不同
// 文本/调用的乱序完成不会丢失早期回放,顺序完成的重复文本也不会全部
// 绑定到同一个位置。只有重复文本且无父关系的并行分支仍有歧义。
...
func insertReplayItems(body []byte, replayItems [][]byte) ([]byte, bool) {
	...
	for _, raw := range input {
		...
		if json.Unmarshal(raw, &item) != nil || item == nil {
			// Never compact the positional index by dropping malformed items.
			return body, false
		}
	...
	injected := 0
	// Capture completion order can differ from client history order. Match each
	// turn against history independently; reserve consumed positions so equal
	// text in sequential turns still binds to successive occurrences.
	usedAnchors := make(map[int]bool, len(turns))
	for _, turn := range turns {
		anchor := turn.anchorIndex(inputItems, 0)
		for anchor >= 0 && usedAnchors[anchor] {
			anchor = turn.anchorIndex(inputItems, anchor+1)
		}
		if anchor < 0 {
			continue
		}
