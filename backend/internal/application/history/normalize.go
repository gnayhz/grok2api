...
func normalizeReasoningItem(raw map[string]json.RawMessage) ([]byte, bool) {
	...
	// Match Grok Build's reasoning InputItem and the client-history sanitizer:
	// status and private output metadata are not input fields. Preserve the
	// ciphertext, id and reasoning content; never reconstruct the ciphertext.
	// This keeps the prefix stable when a client starts resending reasoning
	// that earlier turns had restored from this store.
	input := map[string]json.RawMessage{"type": json.RawMessage(`"reasoning"`)}
	for _, field := range []string{"id", "summary", "content", "encrypted_content"} {
		if value := raw[field]; len(value) > 0 && strings.TrimSpace(string(value)) != "null" {
			input[field] = value
		}
	}
	if _, exists := input["summary"]; !exists {
