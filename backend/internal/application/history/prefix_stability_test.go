...
// At the per-session byte bound, adding a new response must not remove
// reasoning already injected near the beginning of the next request.
func TestReplayBudgetPreservesPreviouslyInjectedPrefix(t *testing.T) {
	store := memory.NewReasoningReplayStore(4)
	replay := New(store, Config{Enabled: true, TTL: time.Hour}, nil)
	ctx := context.Background()
	var input []map[string]any
	input = append(input, map[string]any{"type": "message", "role": "user", "content": "start"})
	var previous []json.RawMessage
	for turn := 0; turn < 8; turn++ {
		text := fmt.Sprintf("answer %d ", turn) + strings.Repeat("reference ", 3000)
		output := []map[string]any{
			{"type": "reasoning", "encrypted_content": validEncrypted(byte(turn + 100))},
			{"type": "message", "role": "assistant", "content": []map[string]any{{"type": "output_text", "text": text}}},
		}
		completed, _ := json.Marshal(map[string]any{"output": output})
		replay.StoreFromCompleted(ctx, "grok-4.6", "budget-prefix", completed)
		input = append(input, output[1], map[string]any{"type": "message", "role": "user", "content": fmt.Sprintf("next %d", turn)})
		body, _ := json.Marshal(map[string]any{"input": input})
		updated := replay.Apply(ctx, "grok-4.6", "budget-prefix", body)
		var got struct {
		...
		for i, item := range previous {
			if i >= len(got.Input) || string(item) != string(got.Input[i]) {
				t.Fatalf("turn %d rewrote earlier upstream input item %d after replay budget filled", turn, i)
			}
		}
		previous = got.Input
		items, _, _ := store.Get(ctx, "grok-4.6", "budget-prefix", time.Now(), time.Hour)
		total := 0
		for _, item := range items {
			total += len(item)
		}
		if total > maxSessionReplayBytes {
			t.Fatalf("replay budget exceeded: %d", total)
		}
	}
}

func TestReplayBudgetRejectsOversizedTurnWithoutPartialReplay(t *testing.T) {
	items := [][]byte{
		[]byte(`{"type":"reasoning","encrypted_content":"` + validEncrypted(212) + `"}`),
		[]byte(`{"type":"message","role":"assistant","content":[{"type":"output_text","text":"` + strings.Repeat("x", maxSessionReplayBytes) + `"}]}`),
	}
	if got := trimReplayTurns(items); len(got) != 0 {
		t.Fatal("oversized turn must not bypass the budget or leave unanchored reasoning")
	}
	first := [][]byte{
		[]byte(`{"type":"reasoning","encrypted_content":"` + validEncrypted(211) + `"}`),
		[]byte(`{"type":"message","role":"assistant","content":[{"type":"output_text","text":"old answer"}]}`),
	}
	got := trimReplayTurns(append(append([][]byte(nil), first...), items...))
	if len(got) != len(first) {
		t.Fatal("oversized new turn displaced old replay")
	}
	for i := range first {
		if string(got[i]) != string(first[i]) {
			t.Fatal("old replay changed")
		}
	}
}

func TestFirstOversizedCompletionRespectsReplayBudget(t *testing.T) {
	store := memory.NewReasoningReplayStore(4)
	replay := New(store, Config{Enabled: true, TTL: time.Hour}, nil)
	payload, _ := json.Marshal(map[string]any{"output": []any{
		map[string]any{"type": "reasoning", "encrypted_content": validEncrypted(219)},
		map[string]any{"type": "message", "role": "assistant", "content": []any{map[string]any{"type": "output_text", "text": strings.Repeat("x", maxSessionReplayBytes)}}},
	}})
	replay.StoreFromCompleted(context.Background(), "grok-4.6", "large-first", payload)
	items, _, _ := store.Get(context.Background(), "grok-4.6", "large-first", time.Now(), time.Hour)
	total := 0
	for _, item := range items {
		total += len(item)
	}
	if total > maxSessionReplayBytes {
		t.Fatalf("first response bypassed byte bound: %d", total)
	}
}

func TestDuplicateCompletionDoesNotAppendOrphanAssistant(t *testing.T) {
	store := memory.NewReasoningReplayStore(4)
	replay := New(store, Config{Enabled: true, TTL: time.Hour}, nil)
	ctx := context.Background()
	payload := []byte(`{"output":[{"type":"reasoning","encrypted_content":"` + validEncrypted(220) + `"},{"type":"message","role":"assistant","content":[{"type":"output_text","text":"same answer"}]}]}`)
	replay.StoreFromCompleted(ctx, "grok-4.6", "duplicate", payload)
	before, _, _ := store.Get(ctx, "grok-4.6", "duplicate", time.Now(), time.Hour)
	for i := 0; i < 3; i++ {
		replay.StoreFromCompleted(ctx, "grok-4.6", "duplicate", payload)
	}
	after, _, _ := store.Get(ctx, "grok-4.6", "duplicate", time.Now(), time.Hour)
	if len(after) != len(before) {
		t.Fatalf("duplicate capture grew replay items from %d to %d", len(before), len(after))
	}
}
