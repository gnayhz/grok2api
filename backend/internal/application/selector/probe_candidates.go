...
func (s *Selector) candidateEligibility(provider account.Provider, upstreamModel, quotaMode string, candidate account.RoutingCandidate, value account.Credential, accountScope clientkeydomain.AccountScope, now time.Time, inference, bypassQualityEligibility, allowQuotaRecovery bool) error {
...
func (s *Selector) qualityProbeCandidates(ctx context.Context, provider account.Provider, modelRouteID uint64, upstreamModel, quotaMode string) ([]uint64, error) {
	...
	for _, c := range values {
		...
		if s.candidateEligibility(provider, upstreamModel, quotaMode, c, value, scope, now, true, true, false) == nil {
