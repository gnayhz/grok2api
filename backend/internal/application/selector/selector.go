...
// RoutingStore 是选号所需的账号仓储能力：路由候选读取、凭据材料物化、
// 健康与模型限制的条件写。账号导入/删除/管理不在选号合同内。
...
func NewSelector(accounts RoutingStore, concurrency repository.ConcurrencyLimiter, sticky repository.StickySessionRepository, tierOrders interface {
	TierOrder(account.Provider, string) []account.WebTier
...
// bypassing only the quality eligibility seam (B2 生产/探针双通道:取证不受资格限制).
...
func (s *Selector) acquirePinned(ctx context.Context, provider account.Provider, accountID, modelRouteID uint64, upstreamModel, quotaMode string, inference, bypassQualityEligibility bool, requestedScope clientkeydomain.AccountScope) (lease *accountLease, err error) {
	...
	criteria := selectionCriteria{provider: provider, modelRouteID: modelRouteID, upstreamModel: upstreamModel,
		...
		allowQuotaRecovery: !bypassQualityEligibility}
	now := time.Now().UTC()
	...
	for _, candidate := range values {
		...
		if err := s.candidateEligibility(provider, upstreamModel, quotaMode, candidate, value, accountScope, now, inference, bypassQualityEligibility, !bypassQualityEligibility); err != nil {
			return nil, err
		}
...
// isRoutingStoreSentinelError 判定存储层业务哨兵错误（未找到/条件写冲突/
// 超限/非法记录/账号池不匹配）。这类错误携带确定性信号，既不允许读旧
// 路由快照，也不算瞬态存储故障。
func isRoutingStoreSentinelError(err error) bool {
	return errors.Is(err, repository.ErrNotFound) || errors.Is(err, repository.ErrConflict) || errors.Is(err, repository.ErrLimitExceeded) || errors.Is(err, repository.ErrInvalidRecord) || errors.Is(err, repository.ErrAccountPoolMismatch)
}
...
func canUseStaleRoutingSnapshot(ctx context.Context, err error) bool {
	...
	if isRoutingStoreSentinelError(err) {
		return false
	}
	// The storage adapter classifies driver-level transient conditions into
	// stable fault kinds at the SQL boundary; this policy only decides which
	// kinds may serve a stale routing snapshot.
	kind, ok := repository.StoreFaultKindOf(err)
	return ok && repository.StoreFaultTransient(kind)
}
...
// 永久错误；仅前者允许按单号跳过继续选号。故障分类经 repository.StoreFaultKindOf
// 归入稳定类别（relational/errors.go 在驱动侧完成分类），类别取舍由
// repository.StoreFaultTransient 单一决定，本函数只负责映射失败标签。
func transientRoutingStoreFailureClass(err error) (string, bool) {
	...
	if kind, ok := repository.StoreFaultKindOf(err); ok && repository.StoreFaultTransient(kind) {
		return "store:" + string(kind), true
	}
	return "", false
}
