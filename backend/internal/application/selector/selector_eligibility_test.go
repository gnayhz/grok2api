...
func TestSelectorOptionalEligibilityAtEveryClaim(t *testing.T) {
	for _, dialect := range []string{"sqlite", "postgres"} {
		t.Run(dialect, func(t *testing.T) {
			...
			// "ordinary"/"segmented" 单次领取路径已删除（生产只剩选号会话）；
			// 其场景由 "session"/"session_segmented" 完整覆盖，这里不再重复执行。
			for _, path := range []string{"pinned", "session", "retry", "session_segmented"} {
				...
				for _, stage := range stages {
					t.Run(path+"/"+stage, func(t *testing.T) {
						...
						if session != nil {
							...
							lease, err = selector.AcquirePinnedForKey(ctx, account.ProviderBuild, values[0].ID, 0, "grok-test", "", false, clientkeydomain.AccountScope{})
						} else {
...
func TestSelectorAuthoritativeAdmissionSupersedesFinalHint(t *testing.T) {
	for _, dialect := range []string{"sqlite", "postgres"} {
		t.Run(dialect, func(t *testing.T) {
			...
			// 单次领取 "ordinary" 路径已删除，"session" 即同一领取合同。
