...
func TestSelectorLayeredCacheUsesLastGoodSnapshotOnTransientLoadFailure(t *testing.T) {
	...
	repo.mu.Lock()
	// The real repository maps driver conditions to store faults at the SQL
	// boundary; the fixture mirrors that contract.
	repo.baseErr = &repository.StoreFault{Kind: repository.StoreFaultConnection, Cause: temporaryRoutingLoadError{message: "temporary base failure"}}
	repo.overlayErr = &repository.StoreFault{Kind: repository.StoreFaultConnection, Cause: temporaryRoutingLoadError{message: "temporary overlay failure"}}
	repo.mu.Unlock()
...
func TestCanUseStaleRoutingSnapshotClassifiesFailuresConservatively(t *testing.T) {
	...
	cancel()
	// The storage adapter classifies driver-level failures at the SQL
	// boundary; the selector only sees the stable repository fault contract.
	// Raw driver-shaped errors reaching this layer stay rejected.
	tests := []struct {
		name string
		ctx  context.Context
		err  error
		...
		{name: "store connection fault", ctx: context.Background(), err: &repository.StoreFault{Kind: repository.StoreFaultConnection, Cause: temporaryRoutingLoadError{message: "retry"}}, want: true},
		{name: "store lock fault", ctx: context.Background(), err: &repository.StoreFault{Kind: repository.StoreFaultLock, Cause: sqliteRoutingLoadError{code: 5}}, want: true},
		{name: "store serialization fault", ctx: context.Background(), err: &repository.StoreFault{Kind: repository.StoreFaultSerialization, Cause: postgresRoutingLoadError{state: "40001"}}, want: true},
		{name: "store timeout fault", ctx: context.Background(), err: &repository.StoreFault{Kind: repository.StoreFaultTimeout, Cause: temporaryRoutingLoadError{message: "slow"}}, want: true},
		{name: "store constraint fault is not stale-worthy", ctx: context.Background(), err: &repository.StoreFault{Kind: repository.StoreFaultConstraint, Cause: sqliteRoutingLoadError{code: 19}}, want: false},
		{name: "wrapped store fault survives error wrapping", ctx: context.Background(), err: fmt.Errorf("load candidates: %w", &repository.StoreFault{Kind: repository.StoreFaultLock, Cause: sqliteRoutingLoadError{code: 6}}), want: true},
		{name: "raw sqlite busy is no longer interpreted here", ctx: context.Background(), err: sqliteRoutingLoadError{code: 5}, want: false},
		{name: "raw postgres state is no longer interpreted here", ctx: context.Background(), err: postgresRoutingLoadError{state: "08006"}, want: false},
		{name: "raw temporary marker is no longer interpreted here", ctx: context.Background(), err: temporaryRoutingLoadError{message: "retry"}, want: false},
		...
		{name: "canceled context wins", ctx: canceledCtx, err: &repository.StoreFault{Kind: repository.StoreFaultConnection, Cause: temporaryRoutingLoadError{message: "retry"}}, want: false},
...
func TestSelectorHydratesOnlyClaimedCredentialAndSkipsStaleCandidate(t *testing.T) {
	...
	session, err := selector.beginSelectionSession(context.Background(), account.ProviderBuild, 0, "model-a", "", "", nil, false)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSelectorReportsNoAccountsWhenEveryCredentialIsStale(t *testing.T) {
	...
	_, err = session.Acquire(context.Background(), nil, false)
	var unavailable *SelectionUnavailableError
...
func TestSelectorPinnedCredentialDoesNotSwitchWhenMaterialIsStale(t *testing.T) {
	...
	_, err := selector.AcquirePinnedForKey(context.Background(), account.ProviderBuild, 1, 0, "model-a", "", true, clientkeydomain.AccountScope{})
	var unavailable *SelectionUnavailableError
	if !errors.As(err, &unavailable) || unavailable.Reason != SelectionNoAccounts {
		t.Fatalf("error = %v, want no accounts", err)
	}
...
func TestSelectorRebindsStickySessionAfterCredentialBecomesStale(t *testing.T) {
	...
	session, err := selector.beginSelectionSession(context.Background(), account.ProviderBuild, 0, "model-a", "", affinity, nil, false)
	if err != nil {
		t.Fatal(err)
	}
	lease, err := session.Acquire(context.Background(), nil, false)
