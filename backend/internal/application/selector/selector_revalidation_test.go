		...
		t.Run(dialect, func(t *testing.T) {
			for _, scenario := range []string{"risk", "tier_scope"} {
				t.Run(scenario, func(t *testing.T) {
					...
					normal, err := func() (*accountLease, error) {
						session, sessionErr := selector.beginSelectionSessionForKey(ctx, v.Provider, 0, "grok-test", "", "", nil, false, scope)
						if sessionErr != nil {
							return nil, sessionErr
						}
						return session.Acquire(ctx, nil, false)
					}()
					if normal != nil {
						normal.Release()
				...
				for _, notify := range []bool{false, true} {
					for _, restriction := range []string{"risk", "tier_scope"} {
						t.Run(fmt.Sprintf("%s/notify=%t/%s", path, notify, restriction), func(t *testing.T) {
							...
							default:
								if s2, serr := selector.beginSelectionSessionForKey(ctx, account.ProviderWeb, 0, "grok-test", "", affinity, nil, false, scope); serr != nil {
									lease, err = nil, serr
								} else {
									lease, err = s2.Acquire(ctx, nil, false)
								}
							}
							if !committed {
			...
			for _, path := range []string{"ordinary", "session"} {
				t.Run(path, func(t *testing.T) {
					...
					if path == "session" {
						...
						if s, serr := selector.beginSelectionSession(ctx, first.Provider, 0, "grok-test", "", affinity, excluded, false); serr != nil {
							lease, err = nil, serr
						} else {
