...
func benchmarkSegmentedSelector(b *testing.B, candidateCount int, enabled, forceFullFallback bool) {
	...
	for b.Loop() {
		...
		session, err := selector.beginSelectionSession(context.Background(), account.ProviderBuild, 0, "benchmark-model", "", "", nil, false)
		if err != nil {
			b.Fatal(err)
		}
...
func TestSegmentedActiveBindsAndReusesStickyAccount(t *testing.T) {
	...
	firstSession, err := selector.beginSelectionSession(context.Background(), account.ProviderBuild, 0, "model", "", "sticky-session", nil, false)
	if err != nil {
		t.Fatal(err)
	}
	first, err := firstSession.Acquire(context.Background(), nil, false)
	if err != nil {
		t.Fatal(err)
	}
	...
	secondSession, err := selector.beginSelectionSession(context.Background(), account.ProviderBuild, 0, "model", "", "sticky-session", nil, false)
	if err != nil {
		t.Fatal(err)
	}
	second, err := secondSession.Acquire(context.Background(), nil, false)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSegmentedActiveSkipsPinnedAndSmallPools(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			var err error
			if test.pinned {
				lease, err = selector.AcquirePinnedForKey(context.Background(), account.ProviderBuild, 1, 0, "model", "", true, clientkeydomain.AccountScope{})
			} else {
