...
func TestSelectorPrioritizesDueQuotaProbeOnce(t *testing.T) {
	...
	session, err := selector.beginSelectionSession(ctx, account.ProviderBuild, 0, "grok-test", "", "", map[uint64]bool{}, true)
	if err != nil {
		t.Fatal(err)
	}
	...
	probeExcluded := map[uint64]bool{probe.ID: true}
	session, err = selector.beginSelectionSession(ctx, account.ProviderBuild, 0, "grok-test", "", "", probeExcluded, false)
	if err != nil {
		t.Fatal(err)
	}
	lease, err = session.Acquire(ctx, probeExcluded, false)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSelectorSkipsQuotaProbeBeforeDue(t *testing.T) {
	...
	selector := NewSelector(accounts, memory.NewConcurrencyLimiter(), memory.NewStickyStore(), nil, time.Hour, time.Second, time.Minute)
	if _, err := selector.beginSelectionSession(ctx, account.ProviderBuild, 0, "grok-test", "", "", map[uint64]bool{}, true); err == nil {
...
func TestSelectorModelQuotaPreservesSessionAffinity(t *testing.T) {
	...
	session, err := selector.beginSelectionSession(ctx, account.ProviderBuild, 0, "model-a", "", "model-a-session", nil, false)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSelectorUsesPaidWeeklyPoolAsWebQuotaGate(t *testing.T) {
	...
	selector := NewSelector(accounts, memory.NewConcurrencyLimiter(), memory.NewStickyStore(), nil, time.Hour, time.Second, time.Minute)
	if _, err := selector.beginSelectionSession(ctx, account.ProviderWeb, 0, "", "fast", "", nil, false); err == nil {
	...
	session, err := selector.beginSelectionSession(ctx, account.ProviderWeb, 0, "", "fast", "", nil, false)
	if err != nil {
		t.Fatal(err)
	}
	lease, err := session.Acquire(ctx, nil, false)
...
func TestSelectorClaimsPaidBillingProbeAfterPeriodEnd(t *testing.T) {
	...
	selector := NewSelector(accounts, memory.NewConcurrencyLimiter(), memory.NewStickyStore(), nil, time.Hour, time.Second, time.Minute)
	session, err := selector.beginSelectionSession(ctx, account.ProviderBuild, 0, "", "", "", map[uint64]bool{}, true)
	if err != nil {
		t.Fatal(err)
	}
	lease, err := session.Acquire(ctx, map[uint64]bool{}, true)
...
func TestSelectorOnlyUsesAccountsSupportingRequestedModel(t *testing.T) {
	...
	session, err := selector.beginSelectionSession(ctx, account.ProviderBuild, 0, "grok-premium", "", "", map[uint64]bool{}, true)
	if err != nil {
		t.Fatal(err)
	}
	lease, err := session.Acquire(ctx, map[uint64]bool{}, true)
...
func TestSelectorKeepsWebQuotaModesIsolated(t *testing.T) {
	...
	selector := NewSelector(accounts, memory.NewConcurrencyLimiter(), memory.NewStickyStore(), nil, time.Hour, time.Second, time.Minute)
	if _, err := selector.beginSelectionSession(ctx, account.ProviderWeb, 0, "grok-chat", "fast", "", nil, false); err == nil {
		t.Fatal("exhausted fast mode should not be selected")
	}
	session, err := selector.beginSelectionSession(ctx, account.ProviderWeb, 0, "grok-chat-auto", "auto", "", nil, false)
	if err != nil {
		t.Fatal(err)
	}
	lease, err := session.Acquire(ctx, nil, false)
...
func TestSelectorUsesTierSpecificWebImageEditQuotaAndPrefersBasic(t *testing.T) {
	...
	session, err := selector.beginSelectionSession(ctx, account.ProviderWeb, 0, "imagine-image-edit", account.QuotaModeWebImageEdit, "", nil, false)
	if err != nil {
		t.Fatal(err)
	}
	lease, err := session.Acquire(ctx, nil, false)
...
func TestSelectorAllowsBasicOnlyForConfirmedWebVideoQuota(t *testing.T) {
	...
	session, err := selector.beginSelectionSession(ctx, account.ProviderWeb, 0, "grok-imagine-video", account.QuotaModeWebVideo720p, "", nil, false)
	if err != nil {
		t.Fatal(err)
	}
	lease, err := session.Acquire(ctx, nil, false)
	...
	lease.Release()
	if _, err := selector.beginSelectionSession(ctx, account.ProviderWeb, 0, "grok-imagine-video", account.QuotaModeWebVideo, "", nil, false); err == nil {
...
func TestSelectorHonorsWebTierPoolOrderBeforeAccountPriority(t *testing.T) {
	...
	selector.UpdatePreferFreeBuild(true)
	session, err := selector.beginSelectionSession(ctx, account.ProviderWeb, 0, "fast-prefer-best", "fast", "", nil, false)
	if err != nil {
		t.Fatal(err)
	}
	lease, err := session.Acquire(ctx, nil, false)
...
func TestSelectorEnforcesClientKeyAccountScopeAcrossProvidersAndTiers(t *testing.T) {
	...
	assertSelected := func(provider account.Provider, tiers clientkeydomain.TierScope, excluded map[uint64]bool, want uint64) {
		...
		session, sessionErr := selector.beginSelectionSessionForKey(ctx, provider, 0, "", "", "", excluded, false, scope)
		if sessionErr != nil {
			t.Fatal(sessionErr)
		}
		lease, acquireErr := session.Acquire(ctx, excluded, false)
		if acquireErr != nil {
			t.Fatal(acquireErr)
		}
		defer lease.Release()
	...
	_, err = selector.beginSelectionSessionForKey(ctx, account.ProviderBuild, 0, "", "", "", map[uint64]bool{buildFree.ID: true}, false, freeBuildScope)
	...
	if _, err := selector.beginSelectionSessionForKey(ctx, account.ProviderWeb, 0, "", "", "", nil, false, buildOnlyScope); !errors.As(err, &unavailable) || unavailable.Code() != "client_key_account_scope_unavailable" {
...
func TestSelectorPropagatesConcurrencyStoreFailure(t *testing.T) {
	...
	if _, err := session.Acquire(ctx, map[uint64]bool{}, true); !errors.Is(err, runtimeErr) {
...
func TestSelectorPreferFreeBuildHotReloadAndSaturationFallback(t *testing.T) {
	...
	selector := NewSelector(accounts, memory.NewConcurrencyLimiter(), memory.NewStickyStore(), nil, time.Hour, time.Second, time.Minute)
	session, err := selector.beginSelectionSession(ctx, account.ProviderBuild, 0, "grok-4.5", "", "existing-session", nil, false)
	if err != nil {
		t.Fatal(err)
	}
	lease, err := session.Acquire(ctx, nil, false)
	...
	selector.UpdatePreferFreeBuild(true)
	stickySession, err := selector.beginSelectionSession(ctx, account.ProviderBuild, 0, "grok-4.5", "", "existing-session", nil, false)
	if err != nil {
		t.Fatal(err)
	}
	stickyLease, err := stickySession.Acquire(ctx, nil, false)
	if err != nil {
		t.Fatal(err)
	}
	...
	freeSession, err := selector.beginSelectionSession(ctx, account.ProviderBuild, 0, "grok-4.5", "", "new-session", nil, false)
	if err != nil {
		t.Fatal(err)
	}
	freeLease, err := freeSession.Acquire(ctx, nil, false)
	if err != nil {
		t.Fatal(err)
	}
	...
	fallbackSession, err := selector.beginSelectionSession(ctx, account.ProviderBuild, 0, "grok-4.5", "", "", nil, false)
	if err != nil {
		t.Fatal(err)
	}
	fallbackLease, err := fallbackSession.Acquire(ctx, nil, false)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSelectorWaitsBrieflyForAccountCapacity(t *testing.T) {
	...
	firstSession, err := selector.beginSelectionSession(ctx, account.ProviderBuild, 0, "model", "", "", nil, false)
	if err != nil {
		t.Fatal(err)
	}
	...
	go func() {
		session, sessionErr := selector.beginSelectionSession(ctx, account.ProviderBuild, 0, "model", "", "", nil, false)
		if sessionErr != nil {
			resultCh <- result{err: sessionErr}
			return
		}
		lease, acquireErr := session.Acquire(ctx, nil, false)
...
func TestSelectorStickySessionWaitsForBoundAccountCapacity(t *testing.T) {
	...
	resultCh := make(chan result, 1)
	go func() {
		session, sessionErr := selector.beginSelectionSession(ctx, account.ProviderBuild, 0, "model", "", "stable-affinity", nil, false)
		if sessionErr != nil {
			resultCh <- result{err: sessionErr}
			return
		}
...
func TestSelectorStickySessionTemporaryFallbackDoesNotRebind(t *testing.T) {
	...
	temporary.Release()
	first.Release()
	resumedSession, err := selector.beginSelectionSession(ctx, account.ProviderBuild, 0, "model", "", "stable-affinity", nil, false)
	if err != nil {
		t.Fatal(err)
	}
	resumed, err := resumedSession.Acquire(ctx, nil, false)
...
func TestSelectorStickyHitRefreshesTTL(t *testing.T) {
	...
	second, err := secondSession.Acquire(ctx, nil, false)
	if err != nil {
		t.Fatal(err)
	}
	second.Release()
...
func TestSelectorAppliesPersistedCooldownOnlyToMatchingModel(t *testing.T) {
	...
	selector := NewSelector(accounts, memory.NewConcurrencyLimiter(), memory.NewStickyStore(), nil, time.Hour, time.Second, time.Minute)
	if _, err := selector.beginSelectionSession(ctx, account.ProviderBuild, 0, "limited-model", "", "", nil, false); err == nil {
	...
	otherSession, err := selector.beginSelectionSession(ctx, account.ProviderBuild, 0, "other-model", "", "", nil, false)
	if err != nil {
		t.Fatalf("other model was blocked: %v", err)
	}
	lease, err := otherSession.Acquire(ctx, nil, false)
	if err != nil {
		t.Fatalf("other model was blocked: %v", err)
	}
	lease.Release()
