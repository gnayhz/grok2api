...
Manual resource checks use `ProbeBuildTarget` to observe the configured Build origin's `/cdn-cgi/trace` through a fixed node and the generation account's transport identity. It sends no account authorization header, bounds response bytes and duration, and uses the existing owned probe task and lease budgets. It returns only an address digest, family and binding revision without updating node health or IP epochs. The quality application compares observations before and after generation; rotating nodes, unsupported traces or changed paths remain inconclusive. General dual-family reachability probes are not substituted for this destination-facing observation.
