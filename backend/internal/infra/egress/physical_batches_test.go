...
func TestPhysicalBatchGateStopsBeforeSubmissionAndClosesInput(t *testing.T) {
	...
	ctx := physical.WithPhysicalCallBatches(ledgerContext(), time.Now().Add(time.Minute), func(context.Context) error { return errors.New("proxyconnect receipt database unavailable") })
	...
	if !neterror.RejectedBeforeSubmission(err) || calls.Load() != 0 || !body.closed || physical.PhysicalCallCount(ctx) != 0 {
		t.Fatalf("err=%v calls=%d bodyClosed=%v entries=%d", err, calls.Load(), body.closed, physical.PhysicalCallCount(ctx))
	}
...
func TestPhysicalBatchesNeverPruneUnacknowledgedBodies(t *testing.T) {
	...
	defer upstream.Close()
	ctx := physical.WithPhysicalCallBatches(ledgerContext(), time.Now().Add(time.Minute), func(context.Context) error { return nil })
	...
	if calls.Load() != physical.MaxPhysicalCalls || len(physical.PhysicalFacts(ctx)) != physical.MaxPhysicalCalls {
		t.Fatal("lost unacknowledged physical fact")
	}
	physical.ConfirmPhysicalFacts(ctx, physical.PhysicalFacts(ctx))
	if physical.PhysicalCallCount(ctx) != physical.MaxPhysicalCalls || len(physical.PhysicalObservations(ctx)) != 0 {
	...
	if calls.Load() != physical.MaxPhysicalCalls+1 || len(facts) != 1 || facts[0].BodyBytes != 4 || physical.PhysicalCallCount(ctx) != physical.MaxPhysicalCalls+1 {
...
func TestPhysicalBatchOwnerCannotBeReplaced(t *testing.T) {
	...
	ctx := physical.WithPhysicalCallBatches(ledgerContext(), time.Now().Add(time.Minute), func(context.Context) error { return rejected })
	ctx = physical.WithPhysicalCallBatches(ctx, time.Now().Add(time.Hour), func(context.Context) error { return nil })
