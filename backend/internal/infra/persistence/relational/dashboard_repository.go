...
const dashboardUsageAggregateSelect = `
	...
	COALESCE(SUM(CASE WHEN first_token_ms IS NOT NULL AND output_tokens > 0 AND duration_ms > first_token_ms THEN CASE WHEN duration_ms - first_token_ms < first_token_ms AND duration_ms - first_token_ms < 1000 THEN duration_ms ELSE duration_ms - first_token_ms END ELSE 0 END), 0) AS generation_total_ms`
