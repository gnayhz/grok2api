...
func (r *DashboardRepository) Snapshot(ctx context.Context, window repository.DashboardSnapshotWindow, snapshotAt time.Time) (dashboarddomain.Aggregate, error) {
	...
	err := r.db.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Model(&requestAuditModel{}).
			Select("provider, COUNT(*) AS requests, COALESCE(SUM(CASE WHEN status_code >= 200 AND status_code < 300 AND COALESCE(error_code, '') = '' THEN 1 ELSE 0 END), 0) AS successful_requests, COALESCE(SUM(total_tokens), 0) AS tokens").
			Where("created_at >= ? AND created_at < ?", start, end).
			Group("provider").
