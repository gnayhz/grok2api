// Package sqlfault maps driver failures into the shared storage fault contract.
package sqlfault
...
// Classify is the single SQL boundary that translates driver-level
...
func Classify(err error) *repository.StoreFault {
...
// Wrap preserves semantic errors and caller cancellation when no SQL class applies.
func Wrap(err error) error {
	if err == nil {
		return nil
	}
	if _, ok := repository.StoreFaultOf(err); ok {
		return err
	}
	if fault := Classify(err); fault != nil {
		return fault
	}
	return err
}
