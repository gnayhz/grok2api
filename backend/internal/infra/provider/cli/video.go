...
func (a *Adapter) GenerateVideo(ctx context.Context, request provider.VideoRequest) (provider.VideoResult, error) {
	if total := buildVideoImageCount(request); total > buildVideoMaxImages {
		return provider.VideoResult{}, provider.WrapVideoStage(provider.VideoStageCreate, 0, fmt.Errorf("Build grok-imagine-video-1.5 最多支持 %d 张输入图，当前为 %d 张", buildVideoMaxImages, total))
	}
	...
	if createErr == nil {
		...
		if parseErr != nil {
			return provider.VideoResult{}, provider.WrapVideoStage(provider.VideoStageCreate, 0, parseErr)
		}
	...
	if !asVideoUpstreamError(createErr, &upstream) || !isHTTPForbidden(upstream.status) {
		return provider.VideoResult{}, provider.WrapVideoStage(provider.VideoStageCreate, 0, createErr)
	}
...
func (a *Adapter) generateVideoOnXAI(ctx context.Context, request provider.VideoRequest, credential account.Credential, accessToken string, recordFallback bool) (provider.VideoResult, error) {
	...
	if issuer == nil {
		return provider.VideoResult{}, provider.WrapVideoStage(provider.VideoStageCreate, 0, fmt.Errorf("XAI 视频需要媒体上传接收服务"))
	}
