		...
		for _, streaming := range []bool{false, true} {
			t.Run(fmt.Sprintf("%s/stream=%v", operation, streaming), func(t *testing.T) {
				server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
					...
					if streaming {
						...
						for _, data := range []string{
							...
							`{"type":"response.output_text.delta","item_id":"msg_1","delta":"matrix answer STOP hidden"}`,
						...
						_, _ = io.WriteString(w, `{"id":"resp_matrix","model":"grok-4.3","status":"completed","output":[{"type":"reasoning","content":[{"type":"reasoning_text","text":"matrix thought"}]},{"type":"message","content":[{"type":"output_text","text":"matrix answer STOP hidden"}]}],"usage":{"input_tokens":9,"output_tokens":7,"total_tokens":16}}`)
					}
				}))
				defer server.Close()
				...
				if operation == conversation.OperationMessages {
					body["stop_sequences"] = []string{" STOP"}
				...
				if operation != conversation.OperationResponses && strings.Contains(string(output), "hidden") {
					t.Fatalf("stop sequence ignored: %s", output)
				}
				if streaming {
