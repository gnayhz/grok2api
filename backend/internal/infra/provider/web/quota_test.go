...
func TestDecodeImagineQuotaSnapshotMatchesObservedProtocol(t *testing.T) {
	...
	if windows[0].Mode != account.QuotaModeWebImagePro || windows[0].Remaining != 4 || windows[0].Total != 0 || windows[0].ResetAt == nil || !windows[0].ResetAt.Equal(wantResetAt) {
		t.Fatalf("image_pro = %#v", windows[0])
	}
	if windows[1].Mode != account.QuotaModeWebVideo720p || windows[1].Remaining != 1 || windows[1].Total != 0 || windows[1].ResetAt == nil || !windows[1].ResetAt.Equal(wantResetAt) {
