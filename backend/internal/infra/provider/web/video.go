...
func (a *Adapter) GenerateVideo(ctx context.Context, request provider.VideoRequest) (provider.VideoResult, error) {
	...
	for _, rawReference := range rawReferences {
		...
		if referenceErr != nil {
			return provider.VideoResult{}, provider.WrapVideoStage(provider.VideoStageCreate, 0, referenceErr)
		}
	...
	if len(segments) == 0 {
		return provider.VideoResult{}, provider.WrapVideoStage(provider.VideoStageCreate, 0, fmt.Errorf("duration 必须在 1 到 15 秒之间"))
	}
	...
	if result.URL == "" {
		return provider.VideoResult{}, provider.WrapVideoStage(provider.VideoStageCreate, 0, fmt.Errorf("视频生成完成但没有返回内容 URL"))
	}
	return result, nil
}
