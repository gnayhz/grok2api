...
func TestReasoningReplayStoreSetGetDelete(t *testing.T) {
	...
	got, ok, err := store.Get(ctx, "grok", "sess", now)
	...
	if _, ok, err := store.Get(ctx, "grok", "sess", now); err != nil || ok {
...
func TestReasoningReplayStoreEvictsOldest(t *testing.T) {
	...
	if _, ok, _ := store.Get(ctx, "m", "a", now); ok {
		t.Fatal("oldest entry should be evicted")
	}
	if _, ok, _ := store.Get(ctx, "m", "c", now); !ok {
