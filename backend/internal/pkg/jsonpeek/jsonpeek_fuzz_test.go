...
// FuzzRootStringFieldScan compares complete root fields with encoding/json,
// including whitespace, escaped keys and duplicate keys.
func FuzzRootStringFieldScan(f *testing.F) {
	...
	f.Add([]byte(`{"a": 0, "\u0074ype":"old", "type":"r\u0069ght"}`), "type")
	f.Fuzz(func(t *testing.T, data []byte, key string) {
...
func FuzzRootIntFieldScan(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, data []byte, key string) {
		...
		var want int64
		if bytes.Equal(raw, []byte("null")) || json.Unmarshal(raw, &want) != nil {
			if ok {
