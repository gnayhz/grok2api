...
| 主动资源检测 | 管理员主动选择账号/出口，用于检查资源或二次确认 | 完整流、token 差分、跨资源与前后正对照；输出正常、异常或无法判定 | 独立任务和报告，不给案件投票、不自动立案或解封，不改变实时守卫阈值 |
...
以上是已观察行为与判别边界，不是跨模型、跨时间的准确率保证。上游变更后应重新建立正常基线；模型、协议、工具或推理档位变化时不能直接复用旧数字。实际账号/出口映射、原始轨迹及运行报告保存在仓库外，仅维护脱敏机制说明和最小人工构造回归。
...
账号页的“可用性检测”检查凭据、额度和普通响应；账号和代理节点页的“风控检测”测量指定资源在 Build 推理面上的可见思考与 token 指纹，不需要先产生案件。行菜单和批量工具栏共用 `entities/guard/ResourceCheckDialog`，每批最多 32 个资源。`POST /api/admin/v1/quality/resource-checks` 接收 `{"kind":"account|node","resource_ids":["1","2"],"model":"公开模型 ID"}`，资源与任务 ID、身份及版本以十进制字符串传输，POST 也兼容整数 ID 输入；202 逐项返回任务 ID 或失败码；部分接受明确保留，重试复用该资源已有的在途任务。GET 使用 `kind` 和逗号分隔 `resource_ids`，每资源返回最近 5 次报告及在途进度。`management.ResourceChecks` 拥有候选准备与提交，`investigator` 拥有有界实验执行，`model` 拥有归因规则，gateway 拥有生成和资源取得，egress 拥有目标路径核实。检查不改写案件、启停或质量限制。

`resource-quality-check-v1` 每组先在控制账号×控制出口执行 `a×512 / a×2048 / a×512` 三次独立会话，固定前缀 `Reply only OK. Data: `、无工具、`low`、256 token 输出上限。三次成功完成且 EOF、两次短输入 token 相同、路径稳定、有可见思考，才建立正常对照。然后只改变一个变量：账号检查换成目标账号，出口检查换成目标节点。目标测量后再次发送控制短样本，核实对照没有漂移。差分吸收固定前缀的常量变化；不硬编码 token 阈值，不把全池多数派当作正常。完整流最多 1 MiB，每次含资源等待与前后路径核实最多 45 秒，每资源最多 4 组、28 次测量、12 分钟。小输出上限不是隐藏推理的费用保证。

两组独立目标结果均有思考且 token 差分匹配正常对照，显示“未发现资源风控异常”。至少三组均无思考且差分不同、对照前后正常，才显示“确认资源异常”：账号方向按实际出口摘要去重，且至少两个独立控制账号；出口方向按账号身份组去重，目标地址与 binding 必须保持不变。相反的已完成目标观测不被其它票数抵消；token 与思考冲突、对照漂移、错误、不完整、取消及路径无法核实均保留为无法判定。资源异常表示当前模型/时间的可复现路由行为，不声称读取上游风控表或证明权重身份。

路径核实使用 `egress.Manager.ProbeBuildTarget`，以生成使用的账号身份和 Build 传输访问已配置上游同域 `/cdn-cgi/trace`，不携带账号认证头，比较生成前后的地址族、地址摘要及代理 binding。目标不支持 trace、节点变更、每请求旋转或未观察到指定路径时弃权，不用普通查 IP 服务的 IPv4 结果冒充访问上游的实际地址。不同地址表示不同可观察路径，不保证机房独立。旋转节点直接报告无法核实固定出口，不消耗生成请求给整个池下结论。

资源检测与旧单账号检查共享全局 32 个在途名额；资源检测最多两项同时 running，跨实例原子认领，其余持久排队。排队超过 4 小时取消，运行沿用 owner/租约/续租，进度与终态写入都检查有效 owner；关闭浏览器不撤销已接受工作，失联任务保留已记录进度并取消。法院任务仍使用两分钟截止，不被资源检测的较长截止放宽。资源检测不进入法院票数或案件证据投影。
...
升级为 `q_probe_task` 增加 `check_report_json` 文本列，并将方向 CHECK 扩至 `account_check`、`resource_check`。资源计划随版本化 `experiment_json` 保存，报告与进度使用独立文本列。SQLite 在事务中重建约束、复制全部已有字段并恢复索引；PostgreSQL 在现有升级事务和 advisory lock 内替换约束。从旧两方向或仅含 `account_check` 的数据库升级均保留历史任务、报告、租约、案件和证据。先排空旧 worker 再升级；旧程序不认识手动检查方向，不能混跑。回退前停止提交并完成或取消在途检查，保留新报告列和记录，不收紧 CHECK 或删除数据来适配旧版本。
