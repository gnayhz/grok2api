...
两个方向使用同一个 Build 探测入口和冻结的 `reasoning-capability-v1` 实验样本，继续由生产守卫分类。样本从三个固定推理题中按触发身份选择；主测与控制必须匹配原模型、规范化 reasoning effort、协议、无工具轮廓、策略 revision 和规则版本。缺失物理身份、规格不支持或策略变化的结果不可采。clean 还要求正常读取到成功的 response.completed。账号资格、凭据、配置与业务面的失败不等同于响应异常。匹配控制的账号与出口随计划持久化，主测和匹配结果分别存储。探测针对同一账号身份串行，防止调查并发制造假象。
...
court 使用有 owner 的跨实例协调租约，事务内再次验证所有者。过期案件先于流量聚合、新开案与维护工作处理，每案最多 1 秒且只占总轮次预算的一部分；处理游标持久化，慢案件不会反复占据队首。仍有大量积压时需要多个轮次，数据库整体不可写时无法保证释放成功。
