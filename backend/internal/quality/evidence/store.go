...
// Config 是证据局运行配置(B3 决议1:观测保留期面板可调,默认 7 天;
// 统计窗与保留期独立)。
type Config struct {
...
// DefaultConfig 返回默认配置:统计窗与旧主窗一致,保留期按 B3 决议1。
func DefaultConfig() Config {
	return Config{Window: 30 * time.Minute, Retention: 7 * 24 * time.Hour, MinWitnessObs: 1}
}
...
// qObservationModel 证据观测表(B3)。本表归证据局所有(registry 迁移
// 其余八表);与底座同库不设跨层外键(D2/D4)。
...
func (s *Store) SetConfigContext(ctx context.Context, cfg Config) error {
	s.reloadMu.Lock()
...
// New 构建证据局存储:建表并从库回填滑窗(I17 同款语义——库为真相)。
// db 来自质量层共享连接(registry.DB())。
func New(ctx context.Context, db *gorm.DB, cfg Config) (*Store, error) {
	cfg = cfg.normalized()
	if db == nil {
		return nil, errNilDB
	}
	if err := db.WithContext(ctx).AutoMigrate(&qObservationModel{}); err != nil {
		return nil, err
	}
...
func (s *Store) maybeSweep(ctx context.Context, now time.Time) {
	...
	if err := s.CleanExpired(ctx, now); err != nil {
		if logger := s.config().Logger; logger != nil {
...
func (s *Store) CleanExpired(ctx context.Context, now time.Time) error {
	...
	return s.db.WithContext(ctx).Where("at < ?", cutoff).Delete(&qObservationModel{}).Error
}

// Count 返回库中观测总数(面板/测试)。
func (s *Store) Count(ctx context.Context) (int64, error) {
	var total int64
	err := s.db.WithContext(ctx).Model(&qObservationModel{}).Count(&total).Error
	return total, err
}
