...
func TestCopyStreamWritesTerminalOnIdleTimeout(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "responses", protocol: streamProtocolResponses, want: []string{`"type":"response.failed"`, `"id":"resp_abort"`, `"created_at":`, `"sequence_number":`, `"code":"server_error"`, `"message":"upstream_stream_idle_timeout: `, `"status":"failed"`, `"model":`}},
...
func TestCopyStreamWritesTerminalOnIncompleteEOF(t *testing.T) {
	...
	got := recorder.Body.String()
	if !strings.Contains(got, `"type":"response.failed"`) || !strings.Contains(got, `"message":"upstream_stream_incomplete: `) {
