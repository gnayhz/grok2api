...
const navigation = [
	...
	{ href: "/guard", label: "nav.guard", icon: ShieldCheck },
	{ href: "/proxies", label: "nav.proxies", icon: Waypoints },
...
const documentation = [
	...
	},
	{
		...
		items: [
			...
			{
				href: "/docs/voice/audio-speech",
				label: "OpenAI Speech",
				method: "POST",
			},
			{
				href: "/docs/voice/audio-tasks",
				label: "OpenAI Audio Tasks",
				method: "POST",
			},
			{
				href: "/docs/voice/audio-transcriptions",
		...
		<div className="flex h-9 items-center gap-1 px-2.5">
			...
			<DropdownMenu>
				...
				>
					...
					<DropdownMenuSeparator />
					<DropdownMenuItem className="h-8" onClick={() => { void logout().catch((error: unknown) => toast.error(error instanceof Error ? error.message : t("errors.generic"))); }}>
						<LogOut />
						{t("auth.signOut")}
					</DropdownMenuItem>
				</DropdownMenuContent>
