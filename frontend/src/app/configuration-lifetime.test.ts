...
let TooltipProvider: typeof import("@/shared/ui/tooltip").TooltipProvider;
...
const tunables = JSON.parse(readFileSync(new URL("../features/guard/__fixtures__/settings-versioned.json", import.meta.url), "utf8"));
...
for (const [domain, actions] of [["gateway", ["save", "reset", "rotation"]], ["guard", ["save", "reset"]], ["tunables", ["save"]]] as const) {
  for (const action of actions) test(`${domain} ${action}: leaving the editor cancels transport and ignores a late acknowledgement`, async (t) => {
    ...
    function TunablesHarness() {
      const settings = useSettings();
      return react.createElement(QualityPage, { settings });
    }
