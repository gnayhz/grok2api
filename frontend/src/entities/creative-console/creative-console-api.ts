import { publicApiErrorFrom, publicApiRequest, publicApiSend, throwPublicApiError, PublicApiError } from "@/shared/api/public-client";
...
export async function generateImage(input: {
  ...
  if (images.length === 0) throw new PublicApiError(200, "The image response did not contain any images", "invalid_response");
...
export async function createVideo(input: {
  ...
  if (!requestId) {
    throw new PublicApiError(200, "The video response did not contain a request ID", "invalid_response");
  }
  return requestId;
}
...
export async function listVoices(input: {
  ...
  if (!isRecord(payload) || !Array.isArray(payload.voices)) {
    throw new PublicApiError(200, "The voice list response was invalid", "invalid_response");
  }
...
export async function synthesizeSpeech(input: {
  ...
  const { response, signal: requestSignal } = await publicApiSend(input.apiKey, "/tts", {
    method: "POST",
  ...
  if (!response.ok) await throwPublicApiError(response, requestSignal);
  if (contentType.includes("application/json")) {
    const payload = await response.json();
    requestSignal.throwIfAborted();
    if (!isRecord(payload) || typeof payload.audio !== "string") {
      throw new PublicApiError(200, "The TTS response was invalid", "invalid_response");
    }
...
export async function transcribeSpeech(input: {
  ...
  const { response, signal: requestSignal } = await publicApiSend(input.apiKey, "/stt", {
    method: "POST",
  ...
  if (!isRecord(payload) || typeof payload.text !== "string") {
    throw new PublicApiError(200, "The STT response was invalid", "invalid_response");
  }
...
async function publicResponsesStream(apiKey: string, body: Record<string, unknown>, onUpdate?: (snapshot: ChatStreamSnapshot) => void, signal?: AbortSignal): Promise<ChatResponseResult> {
  const { response, signal: requestSignal } = await publicApiSend(apiKey, "/responses", {
    method: "POST",
  ...
  if (!response.headers.get("content-type")?.includes("text/event-stream")) {
    ...
    if (!text && !reasoning && tools.length === 0) throw new PublicApiError(response.status, "The Responses API did not return any displayable output", "invalid_response");
    return { text, reasoning, tools };
  }
  if (!response.body) throw new PublicApiError(response.status, "The Responses API stream was empty", "invalid_response");
  ...
  const consume = (block: string) => {
    ...
    if (type === "response.created" || type === "response.in_progress" || type === "response.completed" || type === "response.incomplete") {
      ...
      if (type === "response.incomplete") {
        throw new PublicApiError(response.status, readIncompleteReason(responsePayload) || "The response ended before completion", "incomplete_response");
      }
      return;
    }
    if (type === "response.failed" || type === "error") {
      throw publicApiErrorFrom(response, isRecord(payload.response) ? payload.response : payload, "The Responses API stream failed");
    }
  };
  ...
  try {
    ...
    if (!text.trim() && !reasoning.trim() && tools.size === 0) throw new PublicApiError(response.status, "The Responses API did not return any displayable output", "invalid_response");
    return snapshot();
  } finally {
...
function readVideoStatus(payload: unknown): VideoStatus {
  if (!isRecord(payload) || !isVideoStatus(payload.status)) {
    throw new PublicApiError(200, "The video status response was invalid", "invalid_response");
  }
