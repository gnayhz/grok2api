...
export type ResourceReport = {
  ...
  reason: string; calls: number; max_calls: number; observations?: ResourceObservation[]; results?: ResourceProof[]; generations?: number; path_checks?: number;
};
...
export const resourceReportShape = hasShape({ version: isOneOf("resource-proof-v2"), kind: isOneOf("account", "node"), resource_id: isString, outcome: isOneOf("healthy", "degraded", "inconclusive"), reason: isString, calls: isNumber, max_calls: isNumber,
...
export const resourceChecksDecoder = createValidatedDecoder<{ items: ResourceCheck[] }>("resource checks", hasShape({ items: isArrayOf(hasShape({
  ...
  report: isOptional(resourceReportShape),
})) }));
