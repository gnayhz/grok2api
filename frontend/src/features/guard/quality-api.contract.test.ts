...
// Synthetic, minimal fixtures for current public contracts.
...
test("quality cases fixture decodes (羁押名单/裁决流数据流)", () => {
	...
	if (decoded.items.length === 0) {
		throw new Error("cases fixture is empty — supply a synthetic record");
	}
...
test("quality probes fixture decodes (调查局队列数据流)", () => {
	...
	if (decoded.items.length === 0) {
		throw new Error("probes fixture is empty — supply a synthetic record");
	}
});
...
test("quality egress fixture decodes (出口质量面数据流)", () => {
	...
	if (decoded.nodes.length === 0) {
		throw new Error("egress fixture is empty — supply a synthetic record");
	}
});
...
test("quality settings fixture decodes (仲裁庭参数数据流)", () => {
	...
	if (!decoded.retention || !decoded.evidence_window || !decoded.investigation_timeout) {
