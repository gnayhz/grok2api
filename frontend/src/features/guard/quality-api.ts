...
import { createValidatedDecoder, hasShape, isArrayOf, isBoolean, isNumber, isObject, isOptional, isRecordOf, isString } from "@/shared/api/decoder";

// 质量层管理面:冻结案件/裁决流/评估/探针队列/
// 节点质量面/人工解禁/批量轮换。对应后端 qualityhttp handler。

export type QualityCaseParty = {
	kind: string;
	account_id: number;
	node_id: number;
	epoch: number;
...
export type QualityCaseLive = {
	...
	exit_score?: number;
	account_transport_support?: number;
...
// 证据矩阵(仲裁庭核心视图):行=账号,列=出口(节点+epoch),
// 格=窗口内观测构成(clean/degraded 计数)。
export type QualityMatrixAccount = { id: number; state: string; last_at: string };
export type QualityMatrixExit = { node: number; epoch: number; state: string; last_at: string };
export type QualityMatrixCell = { a: number; n: number; e: number; c: number; d: number };
export type QualityMatrix = {
	accounts: QualityMatrixAccount[];
	exits: QualityMatrixExit[];
	cells: QualityMatrixCell[];
};

export const matrixDecoder = createValidatedDecoder<QualityMatrix>("quality matrix", hasShape({
	accounts: isArrayOf(hasShape({
		id: isNumber,
		state: isString,
		last_at: isString,
	})),
	exits: isArrayOf(hasShape({
		node: isNumber,
		epoch: isNumber,
		state: isString,
		last_at: isString,
	})),
	cells: isArrayOf(hasShape({
		a: isNumber,
		n: isNumber,
		e: isNumber,
		c: isNumber,
...
export function fetchQualityMatrix(signal?: AbortSignal): Promise<QualityMatrix> {
	return apiRequest("/api/admin/v1/quality/evidence/matrix", { signal }, matrixDecoder);
}
...
export type QualityOverview = {
	...
	evidence: { decidable: number; degraded: number; incidence: number; nodes: number; accounts: number };
	observations_total: number;
	/** 主路径观测丢弃计数(满队/落库失败;未注入时缺席) */
	observation_drops?: number | null;
};

const overviewDecoder = createValidatedDecoder<QualityOverview>("quality overview", hasShape({
	cases_total: isNumber,
	cases_open: isNumber,
	verdicts: isRecordOf(isNumber),
	evidence: hasShape({
		decidable: isNumber, degraded: isNumber, incidence: isNumber, nodes: isNumber, accounts: isNumber,
	}),
	observations_total: isNumber,
	observation_drops: isOptional(isNumber),
}));

export function fetchQualityOverview(signal?: AbortSignal): Promise<QualityOverview> {
	return apiRequest("/api/admin/v1/quality/overview", { signal }, overviewDecoder);
}

// G18 批量轮换:仅 webhook 节点;限速内触发,超限如实报告。
export function rotateQualityNodes(nodeIds: number[]): Promise<{ triggered: number; rate_limited: number }> {
	const decoder = createValidatedDecoder<{ triggered: number; rate_limited: number }>("quality rotate", hasShape({
		triggered: isNumber,
		rate_limited: isNumber,
	}));
	return apiRequest("/api/admin/v1/quality/egress/rotate", {
		method: "POST",
