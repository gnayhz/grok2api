...
export function QualityTribunalView() {
	const { t } = useTranslation();
	const cache = useQueryClient();
	const [params] = useSearchParams();
	const navigate = useNavigate();
	const [selected, setSelected] = useState<number | null>(null);
	const selectedId = Number(params.get("case")) || selected;
		...
		<div className="min-w-0">
			...
			<section className="ops-panel">
				...
				{cases.isError ? (
					...
					<div>
						{rows.slice((pageIndex - 1) * 8, pageIndex * 8).map((item) => {
			...
			{selectedId && !chosen && !cases.isPending && (
...
function CaseExperiment({
	...
	const remaining = report
		? Math.max(
				0,
				Math.ceil((Date.parse(report.policy.deadline_at) - now) / 1000),
			)
		: 0;
	return (
		<Dialog
			...
			<DialogContent className="ops-case-dialog">
				...
				<div className="ops-inspector">
					...
					<div className="ops-inspector-rail">
						...
						{!report && (
							<details className="ops-disclosure mt-4">
