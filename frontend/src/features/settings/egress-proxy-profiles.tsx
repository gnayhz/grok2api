...
export function EgressProxyProfiles() {
  ...
  const [editing, setEditing] = useState<EgressProxyProfileDTO | null | undefined>(undefined);
    ...
    <section className="space-y-3">
      <div className="flex items-start justify-between gap-4 px-1">
        <div className="space-y-1">
          <h2 className="text-sm font-medium tracking-tight">{t("egressProxyProfiles.title")}</h2>
          <p className="text-xs leading-5 text-muted-foreground">{t("egressProxyProfiles.description")}</p>
        </div>
      </div>
      <DataTableShell
        toolbar={<><div className="relative w-full sm:w-72"><Search className="pointer-events-none absolute left-2.5 top-1/2 size-3.5 -translate-y-1/2 text-muted-foreground" /><Input className="h-8 pl-8 text-xs" value={search} onChange={(event) => { setSearch(event.target.value); setPage(1); }} placeholder={t("egressProxyProfiles.search")} aria-label={t("egressProxyProfiles.search")} /></div><Button type="button" size="sm" variant="secondary" onClick={openCreate}><Plus />{t("egressProxyProfiles.add")}</Button></>}
      ...
      >
        ...
        {!query.isError ? <Table className="table-fixed">
          <TableHeader><TableRow className="hover:bg-transparent"><TableHead className="w-1/4">{t("egressProxyProfiles.name")}</TableHead><TableHead>{t("egressProxyProfiles.endpoint")}</TableHead><TableHead className="w-28 text-center">{t("egressProxyProfiles.nodes")}</TableHead><TableActionHead /></TableRow></TableHeader>
          {query.isPending ? <TableBody><TableLoadingRow colSpan={4} /></TableBody> : null}
          {!query.isPending && query.data.items.length === 0 ? <TableBody><TableRow><TableCell colSpan={4} className="h-20 text-center text-xs text-muted-foreground">{t(search ? "egressProxyProfiles.noMatches" : "egressProxyProfiles.empty")}</TableCell></TableRow></TableBody> : null}
          {!query.isPending ? <TableBody>{query.data.items.map((profile) => <TableRow key={profile.id}>
            <TableCell><span className="truncate text-xs font-medium" title={profile.name}>{profile.name}</span></TableCell>
      ...
      <Dialog open={editing !== undefined} onOpenChange={(open) => { if (!open) setEditing(undefined); }}>
        <DialogContent className="sm:max-w-[520px]">
          <DialogHeader><DialogTitle>{editing ? t("egressProxyProfiles.editTitle") : t("egressProxyProfiles.addTitle")}</DialogTitle><DialogDescription>{t("egressProxyProfiles.dialogDescription")}</DialogDescription></DialogHeader>
          <form className="space-y-4" onSubmit={(event) => { event.preventDefault(); save.mutate(); }}>
            <div className="space-y-2"><Label htmlFor="proxy-profile-name">{t("egressProxyProfiles.name")}</Label><Input id="proxy-profile-name" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /></div>
            <div className="space-y-2"><Label htmlFor="proxy-profile-url">{t("settings.egress.proxyURL")}</Label><div className="flex gap-2"><Input id="proxy-profile-url" type={proxyVisible ? "text" : "password"} autoComplete="new-password" placeholder={editing ? t("settings.egress.keepConfigured") : "socks5h://user:pass@host:port"} value={form.proxyURL} onChange={(event) => setForm({ ...form, proxyURL: event.target.value })} />{editing ? <Button type="button" variant="outline" size="icon" className="shrink-0" disabled={reveal.isPending} aria-label={t(proxyVisible ? "egressProxyProfiles.hide" : "egressProxyProfiles.reveal")} onClick={() => { if (revealedProxyURL) setProxyVisible((visible) => !visible); else reveal.mutate(); }}>{reveal.isPending ? <Spinner /> : proxyVisible ? <EyeOff /> : <Eye />}</Button> : null}</div><p className="whitespace-pre-line text-xs leading-5 text-muted-foreground">{t("settings.egress.proxyProtocols")}</p></div>
            <DialogFooter><Button type="button" variant="outline" onClick={() => setEditing(undefined)}>{t("common.cancel")}</Button><Button type="submit" disabled={save.isPending || !form.name.trim() || (!editing && !form.proxyURL.trim())}>{save.isPending ? <Spinner /> : null}{t("common.save")}</Button></DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
