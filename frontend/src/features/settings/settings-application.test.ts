...
import type { QualityGuardForm } from "@/features/guard/quality-guard-model";
import type { SettingsSnapshotDTO } from "./settings-api.ts";
import { settingsTestConfig } from "./settings-test-fixture.ts";

type SettingsHook = ReturnType<typeof import("./use-settings.ts").useSettings>;
let dom: JSDOM;
...
test("settings GET forwards cancellation and rejects malformed application metadata", async (t) => {
  const oldFetch = globalThis.fetch;
  t.after(() => { globalThis.fetch = oldFetch; });
  const { getSettings } = await import("./settings-api.ts");
