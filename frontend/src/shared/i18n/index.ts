...
Object.assign(resources.en.translation.settings.egress as unknown as Record<string, string>, {
  ...
  routeRules: "Traffic-class routing", routeRulesHelp: "Pin one upstream call purpose to a dedicated egress node or a direct connection. Rules allocate traffic to a proxy resource — proxy-pool / rotating gateways are valid targets and the exit IP is decided by the proxy service; direct means no proxy at all, including HTTP_PROXY environment variables. Inference and video honor account bindings; auxiliary calls follow the rule even for bound accounts. When a fixed target is disabled, traffic falls back to the scope pool.",
  routeRulesScopeHint: "Currently applied to Grok Build upstream calls. Leave a class unconfigured to keep the existing pool behavior. Inline counters are process-local per instance and reset on restart.", routeRuleNodesFailedRetry: "Failed to load egress nodes — retry", routeRuleNoCandidates: "No eligible egress nodes (need enabled proxy nodes)",
  routeRuleMode: "Routing for {{trafficClass}}", routeRuleNone: "Not configured", routeRuleDirect: "Direct connection", routeRuleFixed: "Fixed node", routeRuleNode: "Target node for {{trafficClass}}", routeRuleDisabled: "Disabled",
  routeClassInference: "Inference requests", routeClassInferenceHelp: "Responses, chat, messages, compaction, and reasoning recovery. Bound accounts keep their bound node; unbound accounts follow this rule.",
  routeClassCredential: "Credential / OAuth", routeClassCredentialHelp: "OAuth token refresh and device authorization against auth.x.ai.",
  routeClassBilling: "Billing queries", routeClassBillingHelp: "Quota, billing, and subscription tier lookups.",
  routeClassModelSync: "Model sync", routeClassModelSyncHelp: "Account model catalog discovery requests.",
  routeClassVideo: "Video tasks", routeClassVideoHelp: "Video job submission, polling, and asset downloads. Bound accounts keep their bound node.",
  routeStatHit: "hit {{count}}", routeStatSkipped: "binding skipped {{count}}", routeStatFallback: "fallback {{count}}", routeStatLastSeen: "last {{time}}",
...
Object.assign(resources["zh-CN"].translation.settings.egress as unknown as Record<string, string>, {
  ...
  routeRules: "流量类别路由",
  routeRulesHelp: "将一类上游调用的用途固定到专属出口节点或直连。规则按代理资源分配流量（代理池/轮换网关同样可选，出口 IP 由代理服务决定）；直连表示不经任何代理（含 HTTP_PROXY 等环境变量代理）。推理与视频尊重账号绑定；辅助调用即使账号已绑定也按规则分流。固定目标被禁用时，流量回退到节点池。",
  routeRulesScopeHint: "当前仅作用于 Grok Build 的上游调用；未配置的类别保持现有节点池行为。行内统计为当前实例的进程内计数，重启后归零。", routeRuleNodesFailedRetry: "出口节点加载失败——重试", routeRuleNoCandidates: "暂无符合条件的出口节点（需启用且已配置代理）",
  routeRuleMode: "{{trafficClass}} 的路由方式",
  routeRuleNone: "未配置",
  routeRuleDirect: "直连",
  routeRuleFixed: "固定节点",
  routeRuleNode: "{{trafficClass}} 的目标节点",
  routeRuleDisabled: "已禁用",
  routeClassInference: "推理请求",
  routeClassInferenceHelp: "Responses、Chat、Messages、上下文压缩与推理恢复。绑定账号仍走绑定节点；未绑定账号按此规则分流。",
  routeClassCredential: "凭证 / OAuth",
  routeClassCredentialHelp: "面向 auth.x.ai 的 OAuth 令牌刷新与设备授权。",
  routeClassBilling: "账单查询",
  routeClassBillingHelp: "额度、账单与订阅档位查询。",
  routeClassModelSync: "模型同步",
  routeClassModelSyncHelp: "账号模型目录发现请求。",
  routeClassVideo: "视频任务",
  routeClassVideoHelp: "视频任务提交、轮询与成品下载。绑定账号仍走绑定节点。",
  routeStatHit: "命中 {{count}} 次",
  routeStatSkipped: "绑定跳过 {{count}} 次",
  routeStatFallback: "回退 {{count}} 次",
  routeStatLastSeen: "最近 {{time}}",
  search: "搜索代理节点",
  healthy: "可用",
  assigned: "已分配账号",
  unassigned: "未分配账号",
