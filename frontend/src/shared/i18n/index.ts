...
Object.assign(resources.en.translation.settings.egress as unknown as Record<string, string>, {
  ...
  routeRulesScopeHint: "Currently applied to Grok Build upstream calls. Leave a class unconfigured to keep the existing pool behavior. Inline counters are process-local per instance and reset on restart.",
...
Object.assign(resources["zh-CN"].translation.settings.egress as unknown as Record<string, string>, {
  ...
  routeRulesScopeHint: "当前仅作用于 Grok Build 的上游调用；未配置的类别保持现有节点池行为。行内统计为当前实例的进程内计数，重启后归零。",
