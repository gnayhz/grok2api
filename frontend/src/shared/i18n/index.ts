...
const resources = {
  "zh-CN": {
    translation: {
      ...
      egressProxyProfiles: {
        title: "共享代理配置",
        description: "将实际代理地址与 Provider 节点分开管理：代理只需维护一次，可同时绑定 Web、Build 和 Console 节点。",
        add: "新增代理配置",
        addTitle: "新增共享代理配置",
        editTitle: "编辑共享代理配置",
        ...
        search: "搜索配置名称",
        empty: "暂无共享代理配置",
        noMatches: "没有匹配的共享代理配置",
        loadingSelection: "正在读取所选配置…",
        selectionUnavailable: "配置 #{{id}} 不可用",
        saved: "共享代理配置已保存",
        deleted: "共享代理配置已删除",
        deleteTitle: "删除共享代理配置？",
        deleteDescription: "将删除“{{name}}”。已绑定节点的配置不能删除。",
        deleteBlocked: "已绑定 {{count}} 个节点",
        assignment: "代理配置",
        assignmentHelp: "可选择共享配置以跨 Provider 共用同一代理地址。健康、容量、User-Agent 与 Clearance 仍按节点独立管理。",
        independent: "独立配置（仅当前节点）",
        managedByProfile: "由所选共享代理配置统一管理",
        nodeCount: "{{count}} 个节点",
        reveal: "显示完整代理地址",
        hide: "隐藏完整代理地址",
  ...
  en: {
    translation: {
      ...
      egressProxyProfiles: { title: "Shared proxy configurations", description: "Manage physical proxy endpoints separately from Provider nodes. Maintain a proxy once and bind it to Web, Build, and Console nodes.", add: "Add proxy configuration", addTitle: "Add shared proxy configuration", editTitle: "Edit shared proxy configuration", dialogDescription: "Changing the proxy URL updates every bound node. Capacity and operational state remain isolated per node.", name: "Configuration name", endpoint: "Proxy endpoint", nodes: "Bound nodes", search: "Search configuration names", empty: "No shared proxy configurations", noMatches: "No matching shared proxy configurations", loadingSelection: "Loading selected configuration…", selectionUnavailable: "Configuration #{{id}} is unavailable", saved: "Shared proxy configuration saved", deleted: "Shared proxy configuration deleted", deleteTitle: "Delete shared proxy configuration?", deleteDescription: "This deletes {{name}}. Configurations that are still bound to nodes cannot be deleted.", deleteBlocked: "Bound to {{count}} nodes", assignment: "Proxy configuration", assignmentHelp: "Select a shared configuration to use one proxy endpoint across Providers. Health, capacity, User-Agent, and Clearance remain isolated per node.", independent: "Independent (this node only)", managedByProfile: "Managed by the selected shared proxy configuration", nodeCount: "{{count}} nodes", reveal: "Reveal full proxy URL", hide: "Hide full proxy URL", revealUnavailable: "Only a saved proxy configuration can reveal its URL" },
