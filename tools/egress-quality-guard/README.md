...
Grok Build probes through each fixed egress node. It can quarantine anomalously
fast or malformed routes and restore them only after a clean recovery probe.
...
1. Passive mode polls recent successful streaming audits and recomputes visible
   throughput as `(output - reasoning) / (duration - first token)`.
...
4. The probe measures visible output tokens, chunk cadence, first-token time,
   instruction-marker compliance, and visible-token throughput.
5. A hard threshold quarantines immediately. A soft threshold or probe error
...
- `passive`: inspect ordinary request audits every few seconds. This adds no
  model requests; recovery probes still run for nodes quarantined by the guard.
...
with fewer than 32 visible tokens, and audits created by the guard's own client
...
The admin sidebar includes a Quality guard page showing service freshness, mode, per-node visible Token/s, time to first token, strike counts, quarantine state, and recent events. Operators can also run one real model quality test against a selected node.

The page also reports cumulative automatic checks, active probes, passive audits, anomaly hits, quarantine and recovery actions, and visible output tokens produced by active probes since statistics were enabled. Manual tests are excluded. Actual proxy transfer bytes cannot be recovered reliably from HTTPS/SSE request audits, so Token counts are not presented as network traffic.
...
- HTTPS/SSE audits cannot provide reliable proxy transfer-byte counts. The UI reports active-probe visible output Tokens and does not label them as network traffic.
- Intermediary buffering can produce unusually high instantaneous Token/s, so thresholds require calibration for each route.
- Passive monitoring processes only complete successful streaming requests with enough visible output to calculate speed. Short and failed requests are ignored.
