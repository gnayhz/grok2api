...
出口质量守护程序同时支持“真实请求审计被动检测”和“固定 Prompt 主动探测”。被动异常只作为
候选信号；只有固定 Prompt 主动复测确认异常后，节点才会被临时隔离。
...
4. 普通真实请求超过 TPS 阈值时只立即触发一次固定 Prompt 主动复测，不能直接隔离节点。
...
- `passive`：轮询普通请求审计本身不消耗模型 Token；发现候选异常时会额外执行一次主动确认探测，守护程序隔离的节点仍会执行恢复探测。
...
- 真实请求可能在输出已有文件、长常量或缓存内容，因此被动高 TPS 只会触发固定 Prompt 复测，不会直接隔离节点。
